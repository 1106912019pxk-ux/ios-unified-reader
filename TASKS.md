...
- [ ] 重新设计自动阅读；当前实验实现已回退，不进入稳定版。
  - 已知问题：从分页阅读切换到滚动阅读会丢失当前位置并跳回章节开头。
  - 已知问题：EPUB 的纯图片 spine（例如封面）会触发文本/图片阅读器切换，自动阅读停止且控制条消失。
...
- [ ] 系统 TTS：如仍需要，作为无需账号的轻量回退引擎；不替代微软/本地模型。
- [x] 可切换 TTS 前台听书（代码完成，待真机验收）：
  - 微软 Azure Speech REST：晓晓、云希，密钥只存本机钥匙串。

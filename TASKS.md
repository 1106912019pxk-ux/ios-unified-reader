...
以下功能已经确认要做，但不进入当前稳定性修复批次：

- [ ] 自动阅读：只用于上下滚动阅读，支持开始、暂停和速度调节。
- [ ] 系统 TTS：先接入 iOS/Readium 可用的系统语音，提供独立听书按钮、语速调节与正文进度跟随。
- [ ] 微软语音：稳定版之后接入，首版只内置或下载 1–2 个语音以控制包体。
- [ ] 后台听书：支持锁屏、切换应用和耳机控制；与微软语音一起作为后续批次处理。

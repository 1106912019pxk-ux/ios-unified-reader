...
final class EBookReaderViewController: UIViewController {
    ...
    private func showError(_ error: Error) {
        ...
        titleLabel.text = "Unable to open e-book"
    ...
    @objc private func toggleBookmark() {
        ...
        if book.bookmarks.contains(where: { $0.locatorJSON == json }) {
            ...
            store.addBookmark(title: locator.title ?? "Bookmark", locatorJSON: json, to: bookID)
        }
    ...
    @objc private func presentContents() {
        ...
        Task { [weak self] in
            ...
            do {
                ...
                showAlert(title: "Contents unavailable", message: error.localizedDescription)
            }
        }
    }
...
extension EBookReaderViewController: EPUBNavigatorDelegate {
    ...
    func navigator(_ navigator: Navigator, presentError error: NavigatorError) {
        showAlert(title: "Reader error", message: error.localizedDescription)
    }
}
                ...
                if section == .contents {
                    ForEach(entries) { entry in
                        Button {
                            ...
                            Text(entry.link.title ?? "Untitled section")
            ...
            Form {
                Section("Appearance") {
                    ...
                    Picker("Font", selection: $preferences.fontFamily) {
                        Text("Book default").tag(String?.none)
                    ...
                    Button {
                        ...
                        Label("Import TTF/OTF font", systemImage: "text.badge.plus")
                    }
                }

                Section("Typography") {
                    valueSlider("Font size", value: $preferences.fontSize, range: 0.7 ... 2, step: 0.05)
                    valueSlider("Line height", value: $preferences.lineHeight, range: 1 ... 2.4, step: 0.05)
                    valueSlider("Page margins", value: $preferences.pageMargins, range: 0 ... 2, step: 0.1)
                    valueSlider("Paragraph spacing", value: $preferences.paragraphSpacing, range: 0 ... 2, step: 0.1)
                }

                Section("Layout") {
                    Toggle("Scrolling", isOn: $preferences.isScrollEnabled)
                    Toggle("Use publisher styles", isOn: $preferences.usesPublisherStyles)
                }
        ...
        .sheet(isPresented: $isImportingFont) {
            DocumentPickerView(
                ...
                do {
                    ...
                    fontMessage = "Font imported. Reopen this book once to make newly imported fonts available to EPUB content."
                } catch {
